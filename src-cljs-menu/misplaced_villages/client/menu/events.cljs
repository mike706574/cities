...
(rf/reg-event-db
 ...
 (fn [{socket :app/socket :as db} _]
   ...
   (if socket
     (do (log/info "Syncing!")
         (.send socket (pr-str {:menu/status :sync}))))
   db))
...
(rf/reg-event-db
 ...
 (fn [{:keys [:app/socket :app/player] :as db} [_ opponent]]
   ...
   (.send socket (pr-str {:menu/status :send-invite :menu/player opponent}))
   db))
...
(rf/reg-event-db
 ...
 (fn [{:keys [:app/player :app/socket] :as db} [_ opponent]]
   ...
   (.send socket (pr-str {:menu/status :accept-invite :menu/player opponent}))
...
(rf/reg-event-db
 ...
 (fn [{:keys [:app/player :app/socket] :as db} [_ opponent]]
   ...
   (.send socket (pr-str {:menu/status :reject-invite :menu/player opponent}))
   (update db :menu/received-invites disj [opponent player])))
...
(rf/reg-event-db
 ...
 (fn [{:keys [:app/player :app/socket] :as db} [_ opponent]]
   ...
   (.send socket (pr-str {:menu/status :cancel-invite :menu/player opponent}))
