  ...
  (handle-exceptions request
    (with-body [invite :cities/invite request]
      (let [[sender recipient] invite
        ...
        (cond
          (not (= player sender)) (body-response 403 request {:cities.server/message "Sending invites for other players is not allowed."})
          (= sender recipient) (body-response 409 request {:cities.server/message "You can't invite yourself."})
          (not (user-api/credentials user-manager recipient)) (body-response 409 request {:cities.server/message (str "Player " recipient " does not exist.")})
          :else
  ...
  (handle-exceptions request
    (let [{{player "player"} :headers
      ...
      (if-not (or sender? (= player recipient))
        (body-response 403 request {:cities.server/message "You can't delete other players' invites."})
    ...
    (with-body [invite :cities/invite request]
      (let [{player "player"} (:headers request)
        ...
        (if-not (= player recipient)
          (body-response 403 request {:cities.server/message "You can't accept invites for other players."})
    ...
    (with-body [move :cities.game/move request]
      (let [{{game-id :id} :params
        ...
        (if-not (= player (:cities.player/id move))
          (body-response 403 request {:cities.server/message "Taking turns for other players is not allowed."})
