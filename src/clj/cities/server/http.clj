...
(defmacro with-body
  ...
  `(or (unsupported-media-type ~request)
       ...
       (let [~body-sym (parsed-body ~request)]
         (if-not ~body-sym
           (body-response 400 ~request {:cities.server/message "Invalid request body representation."})
           (if-let [validation-failure# (spec/explain-data ~body-spec ~body-sym)]
             (body-response 400 ~request {:cities.server/message "Invalid request body."
                                          :cities.server/data validation-failure#})
             ~@body)))))
...
(defmacro handle-exceptions
  ...
  `(try
     ...
     (catch Exception e#
       ...
       (body-response 500 ~request {:cities.server/message "An error occurred."}))))
